...
def myPlanning(startPosition, endPosition, time, startVel, endVel, startAcc, 
               endAcc):
    ...
    for i in range(len(startPosition)):
        X = np.matrix([startPosition[i], endPosition[i],startVel[i],endVel[i],
                       startAcc[i],endAcc[i]]).T
